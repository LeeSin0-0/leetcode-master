...
**解法一调用了StringBuffer类的方法，解法二用的朴素字符反转，都值得学习**
...
class Solution {
    public String reverseStr(String s, int k) {
        ...
        while (start < length) {
            ...
            int firstK = (start + k > length) ? length : start + k;//找到第一段k的末尾处,如果元素不够了则末尾为length
            int secondK = (start + (2 * k) > length) ? length : start + (2 * k);//找到第二段k的末尾处,如果元素不够了则末尾为length
            ...
            if (firstK < secondK) { //此时该次循环的剩余长度一定大于k，即firstk和secondk之间有元素
