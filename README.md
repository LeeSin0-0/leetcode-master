...
3. [栈与队列：225.用队列实现栈](./problems/0225.用队列实现栈.md)（Java中，LinkedList实现了Queue接口，boolean add(E e)， poll()获取并移除， peek()获取但不移除）（双向队列(Deque),是Queue的一个子接口，双向队列是指该队列两端的元素既能入队(offer)也能出队(poll),）
