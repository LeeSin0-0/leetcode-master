...
* 编程素养
    ...
    * [什么是核心代码模式，什么又是ACM模式？](./problems/前序/什么是核心代码模式，什么又是ACM模式？.md)  将java输入输出格式笔记在此处。
...
6. [链表：19.删除链表的倒数第 N 个结点](./problems/0019.删除链表的倒数第N个节点.md)（**双指针的经典应用，如果要删除倒数第n个节点，让fast移动n+1步(fast首先走n + 1步 ，为什么是n+1呢，因为只有这样同时移动的时候slow才能指向删除节点的上一个节点)，然后让fast和slow同时移动，直到fast指向链表末尾。删掉slow所指向的节点就可以了。**）
...
2. [哈希表：242.有效的字母异位词](./problems/0242.有效的字母异位词.md)（简单的数组哈希表）**s是Sting类型， charAt(i) - 'a'代表s的第i位字母（从0开始）**
3. [哈希表：1002.查找常用字符](./problems/1002.查找常用字符.md) **注意String.valueOf(c)就是返回c的字符串形式**
4. [哈希表：349.两个数组的交集](./problems/0349.两个数组的交集.md)**（用set（set是无序且不可重复），且交集是去了重的）**
...
1. [字符串：344.反转字符串](./problems/0344.反转字符串.md)（头尾两个指针相互交换，向中间逼近）(或者用^来完成)
...
8. [字符串：459.重复的子字符串](./problems/0459.重复的子字符串.md)（字符串由子字符串重复构成，求该子字符串，真的比较难，直接copy代码吧）
...
1. [数组：27.移除元素](./problems/0027.移除元素.md)（快慢双指针，数组做过啦）
2. [字符串：344.反转字符串](./problems/0344.反转字符串.md)
3. [字符串：替换空格](./problems/剑指Offer05.替换空格.md)
4. [字符串：151.翻转字符串里的单词](./problems/0151.翻转字符串里的单词.md)（暴力解法是使用Arraylist,）
5. [链表：206.翻转链表](./problems/0206.翻转链表.md)（翻转单链表，简单，pre,cur,temp三个指针）
...
2. [栈与队列：232.用栈实现队列](./problems/0232.用栈实现队列.md)(用了Stack类,posh(),pop(),peek()。用两个栈来实现一个队列。现在java中stack已经快被遗弃了，我们通常把Deque作为Stack使用，注意只调用push()/pop()/peek()方法，不要调用addFirst()/removeFirst()/peekFirst()方法，这样代码更加清晰)
3. [栈与队列：225.用队列实现栈](./problems/0225.用队列实现栈.md)（Java中，LinkedList实现了Queue接口，boolean add(offer)()， poll()获取并移除， peek()获取但不移除）（双向队列(Deque),是Queue的一个子接口，双向队列是指该队列两端的元素既能入队，也能出队。addLast,pollFirst,peekFirst对应了上面queue的操作，还有addFirst，pollLast， peekLast）
